...
def create_app() -> FastAPI:
    ...
    async def live_banner(
        ...
        return RedirectResponse(
            url=f"/render?user={user}&color={color}&t={timestamp}"
        )
    ...
    async def render_banner(
        ...
        """
        try:
            # Generate CSS for visualization bars
            css_bar = generate_css_bar(config.num_bars)
            
            # Fetch last played track from LastFM
            ...
            # Generate timestamp for cache control
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Error rendering banner: {str(e)}"
            )
