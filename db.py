...
def update_local_db(data):
    ...
    try:
        ...
        cursor.execute(
            f"CREATE TABLE IF NOT EXISTS {COLLECTION_NAME} (time INTEGER, song TEXT, artist TEXT, url TEXT, thumbnail TEXT)"
        )
        cursor.execute(
            f"INSERT INTO {COLLECTION_NAME} (time, song, artist, url, thumbnail) VALUES (?, ?, ?, ?, ?)",
            (
                data["time"],
                data["song"],
                data["artist"],
...
if __name__ == "__main__":
    a = SongDetailFetcher()
    test_data = a.get_details(
        "https://music.youtube.com/watch?v=Ydv6usKn2rg&list=RDAMVMYdv6usKn2rg"
    )
